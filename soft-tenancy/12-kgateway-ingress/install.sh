#!/usr/bin/env bash
# Step 12: kgateway as the ingress. One Gateway per tenant (a, b, c), one for the platform.
set -euo pipefail
...
for gw in tenant-a tenant-b tenant-c platform; do
...
echo "    tenant-c  :30183   *.tenant-c.lab"
...
echo "    kubectl --as carol apply -f 12-kgateway-ingress/route-tenant-c.yaml"
echo
